# === AIVANA AutoPilot v8.2 — AutoSync + AutoFolder + LinkVerify ===
...
function Upload-All {
    ...
    foreach ($file in $files) {
        ...
        if ($relative -notmatch "/") { $folderUri = "" } else {
    ...
    # === Verify Key URLs Automatically ===
    Write-Host "`n🌐 Verifying important URLs..."
    $urls = @(
    ...
    foreach ($u in $urls) {
        try {
            ...
            if ($resp -eq 200) { Write-Host "✅ OK: $u" -ForegroundColor Green }
            else { Write-Host "⚠️ HTTP $resp → $u" -ForegroundColor Yellow }
        } catch {
            Write-Host "❌ Missing/blocked: $u" -ForegroundColor Red
        }
    }
