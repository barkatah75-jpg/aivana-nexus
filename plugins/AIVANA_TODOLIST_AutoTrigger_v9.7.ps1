# ===========================================
#  AIVANA_TODOLIST_AutoTrigger_v9.7.ps1
...
function Upload-FileToFtp {
  ...
  try {
    ...
    Write-Host "Retry -> $($localPath | Split-Path -Leaf): $($_.Exception.Message)" -ForegroundColor Yellow
    return $false
  }
}
...
function Run-AutoFix {
  ...
  if ($okCount -ge 6) {
    ...
    Write-Host "⚠️ Verification incomplete ($okCount/7)"
    Send-Telegram "⚠️ AIVANA AutoTrigger: Only $okCount/7 guides online!"
