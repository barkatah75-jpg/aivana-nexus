# === AIVANA AutoPilot v7.9 — RootDirect Mode (Final FTP Fix) ===
...
Write-Host "`n=== Starting AIVANA Auto Deploy (RootDirect) ===" -ForegroundColor Cyan
...
try {
    $test = [System.Net.FtpWebRequest]::Create("$ftpHost/")
    $test.Method = [System.Net.WebRequestMethods+Ftp]::ListDirectory
    $test.Credentials = New-Object System.Net.NetworkCredential($ftpUser, $ftpPass)
    $res = $test.GetResponse()
    Write-Host "🌍 FTP root access confirmed." -ForegroundColor Green
    $res.Close()
} catch {
    Write-Host "❌ FTP root test failed: $($_.Exception.Message)" -ForegroundColor Red
}

# 🚀 Upload all files directly to root (no public_html duplication)
